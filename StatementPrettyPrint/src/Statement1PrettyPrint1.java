...
public final class Statement1PrettyPrint1 extends Statement1 {
    ...
    public void prettyPrint(SimpleWriter out, int offset) {
        ...
        switch (this.kind()) {
            case BLOCK: {
                for (int n = 0; n < this.lengthOfBlock(); n++) {
                    Statement st = this.removeFromBlock(n);
                    st.prettyPrint(out, offset);
                    this.addToBlock(n, st);
                }
                break;
            }
            case IF: {
                Statement st = this.newInstance();
                Condition cd = this.disassembleIf(st);
                String temp = toStringCondition(cd);
                printSpaces(out, offset);
                out.println("IF " + temp + " THEN");
                st.prettyPrint(out, offset + Program.INDENT_SIZE);
                printSpaces(out, offset);
                out.println("END IF");
                this.assembleIf(cd, st);
                break;
            }
            case IF_ELSE: {
                Statement st = this.newInstance();
                Statement sd = this.newInstance();
                Condition cd = this.disassembleIfElse(st, sd);
                String temp = toStringCondition(cd);
                printSpaces(out, offset);
                out.println("IF " + temp + " THEN");
                st.prettyPrint(out, offset + Program.INDENT_SIZE);
                printSpaces(out, offset);
                out.println("ELSE");
                sd.prettyPrint(out, offset + Program.INDENT_SIZE);
                printSpaces(out, offset);
                out.println("END IF");
                this.assembleIfElse(cd, st, sd);
                break;
            }
            case WHILE: {
                Statement st = this.newInstance();
                Condition cd = this.disassembleWhile(st);
                String temp = toStringCondition(cd);
                printSpaces(out, offset);
                out.println("WHILE " + temp + " DO");
                st.prettyPrint(out, offset + Program.INDENT_SIZE);
                printSpaces(out, offset);
                out.println("END WHILE");
                this.assembleWhile(cd, st);
                break;
            }
            case CALL: {
